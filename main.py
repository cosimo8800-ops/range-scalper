...
# CONFIG
# ============================================================
YAHOO_SUFFIX  = "-USD"  # Yahoo Finance crypto suffix
COINGECKO_BASE = "https://api.coingecko.com/api/v3"

# Telegram (valori letti da Streamlit Secrets)
...
VOL_MULTIPLIER   = 1.2          # volume candle >= 1.2x media (yfinance base currency)
...
# Mappa intervalli -> yfinance (rispetta limiti API)
# H1: max 60d | H1 usato anche per 2h/4h con aggregazione
INTERVAL_MAP = {
    "1h": ("1h",  "59d"),
    "2h": ("1h",  "59d"),
    "4h": ("4h",  "59d"),
...
# Mappa simboli speciali per yfinance
YAHOO_SYMBOL_MAP = {
    "WBTUSDT":  "BTC-USD",   # Wrapped BTC = BTC
    "WBTCUSDT": "BTC-USD",
    "HYPEUSDT": "HYPE-USD",
    "SUIUSDT":  "SUI20947-USD",
    "TAOUSDT":  "TAO22974-USD",
    "USD1USDT": None,        # stablecoin, skip
    "USDGUSDT": None,        # stablecoin, skip
    "XAUTUSDT": "XAU-USD",
    "PAXGUSDT": "PAXG-USD",
    "ASTERUSDT": None,
}
...
def get_candles(symbol, interval, limit=100):
    """Fetch OHLCV da Yahoo Finance tramite yfinance (sempre disponibile da cloud)"""
    import yfinance as yf
    # Skip simboli non supportati
    if YAHOO_SYMBOL_MAP.get(symbol) is None and symbol in YAHOO_SYMBOL_MAP:
        return None
    # Usa mappa se disponibile, altrimenti costruisci ticker standard
    if symbol in YAHOO_SYMBOL_MAP:
        ticker = YAHOO_SYMBOL_MAP[symbol]
    else:
    ...
    try:
        ...
        df = df[df["close"] > 0].dropna(subset=["open","high","low","close"]).reset_index(drop=True)
        # Per H2 simula aggregando coppie di candele H1
        if interval == "2h":
            df = df.iloc[::2].reset_index(drop=True)
        # Escludi ultima candela (in formazione, volume incompleto)
        if len(df) > 10:
            df = df.iloc[:-1]
        # Restituisci le ultime N candele
...
def analyze_asset(asset_info, btc_breakdown):
    ...
    # --- EMISSIONE SEGNALE (AND STRICT, SOLO LONG) ---
    ...
    if long_sweep and long_flip and vol_ok:
        ...
        if not vol_ok:
            reasons.append("volume insufficiente")
...
def run_backtest(symbols, months=6, starting_capital=10_000, risk_pct=0.01):
    ...
    # yfinance H1 limit = 59 giorni max
    # Per backtest lungo usiamo 1d candles con logica adattata
    use_daily = months > 2
    if use_daily:
        limit = min(months * 30, 730)
        bt_interval_h1 = "1d"
        bt_interval_d1 = "1d"
    else:
        limit = min(months * 30 * 24, 59 * 24)
        bt_interval_h1 = "1h"
        bt_interval_d1 = "1d"
    ...
    for idx, sym_info in enumerate(symbols):
        ...
        candles_h1 = get_candles(symbol, bt_interval_h1, limit)
        candles_d1 = get_candles(symbol, bt_interval_d1, 200)
