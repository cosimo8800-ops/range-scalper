...
# Mappa intervalli Binance -> CryptoCompare
INTERVAL_MAP = {
    "1h":  ("histohour", 1),
    "2h":  ("histohour", 2),
    "4h":  ("histohour", 4),
...
def get_candles(symbol, interval, limit=100):
    """Fetch OHLCV da CryptoCompare (no API key necessaria)"""
    # symbol es. BTCUSDT -> base=BTC, quote=USDT
    base  = symbol.replace("USDT", "").replace("BUSD", "")
    quote = "USDT"

    endpoint, agg = INTERVAL_MAP.get(interval, ("histohour", 1))

    # Per timeframe settimanale simuliamo con giorni
    actual_limit = limit
    if interval == "1w":
        actual_limit = limit * 7
    ...
    try:
        ...
        # Rimuovi candele con prezzo zero
